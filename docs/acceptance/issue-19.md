...
- snapshot 先写临时文件并 fsync，再原子 rename 并 fsync 父目录；之后才用已 fsync 的空 WAL
  原子替换旧 WAL。snapshot 已 rename 但旧 WAL 尚未轮换时，replay 按
...
| snapshot/compaction crash window | `snapshot_compaction_recovers_before_and_after_wal_rotation` 覆盖未完成 snapshot temp、snapshot 已发布但旧 WAL 尚存、WAL 已轮换。 |
