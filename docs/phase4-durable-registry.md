...
| 层 | 内容 | 持久化路径 |
| --- | --- | --- |
| 权威控制元数据 | GlobalTaskId、状态、Attempt、TaskHandle、Runner generation、ContentId | 64 KiB/record 上限的追加式 registry WAL；按 acceptance mode fsync/复制。 |
...
Registry record 只引用 ContentId，不包含 chunk manifest 或内容字节。`DataTransferQueue` 仅存在于数据
...
receipt 同时报告 requested/actual、状态、已确认元数据副本数和最小输入副本数。当前阶段仍是单控制端，
